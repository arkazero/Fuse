...
public class TransformRouteTest{
	...
	public void testDroppingOrder() {
		try {
			...
			// generate the expected XML for the non-admin order after marshaling
			String nonAdminXML = getExpectedXmlString(testNonAdminOrder);
			//TODO add call to get expected JSON
			
			// set xml and json strings as the expected body for our mock end point to receive
			fulfillmentEndpoint.expectedBodiesReceived(nonAdminXML);
			//TODO set expected JSON
			
			// send test objects to direct end point in route
			orderProducer.sendBody(testNonAdminOrder);

			// assert our expectation XML and JSON matched the XML and JSON bodies received
			fulfillmentEndpoint.assertIsSatisfied();
			//TODO assert order log end point is satisfied
...
//TODO uncomment once filter is in place
//	@Test
	@DirtiesContext
...
//	private String getExpectedJSONString(Order order) throws JsonGenerationException, JsonMappingException, IOException {
