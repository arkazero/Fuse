...
public class TransformRouteTest{
	...
	public void testDroppingUndeliveredOrder() {
		try {
			...
			fulfillmentEndpoint.setExpectedCount(1);
			...
			String orderJSON = fulfillmentEndpoint.getExchanges().get(0).getIn().getBody(String.class);
	...
	public void testDroppingDeliveredOrder() {
		try {
			...
			//Create order object to test with an order that has already been delivered
			Order alreadyDeliveredTestOrder = createTestOrder(true);
			putToQueue(alreadyDeliveredTestOrder);

			//Set expected exchange out received as 0 on both end points as this order should be filtered out
